...
def leitura(cursor, numero, tempo, conx, cursorMySQL):
    ...
    for x in Quantidade:
        ...
        for i in range(temperatura):
            i = i + 1
            for a in range(ociosidade):
                valor = vetorEquip[i-1][a]
                a = a + 1
                cursor.execute("INSERT INTO Leitura (temperatura, ociosidade, momento) VALUES (?,?, ?, ?);",
                               (i, a, dataHoraFormat))
                conn.commit()
    ...
    for x in Quantidade:
        ...
        for i in range(Equipamentos):
            ...
            for a in range(Componentes):
                ...
                cursorMySQL.execute("INSERT INTO Leitura (temperatura, ociosidade, momento) VALUES (%s,%s, %s, %s);",
